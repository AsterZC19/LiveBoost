...
- 双推送：
  - 分速推送：按活动类型间隔推送
  - 时速推送：每个整点推送一次
- 补火提醒：根据 T10 PT 变化自动扣火，并在最后一把前进行文字与语音提示
- 语音 TTS + AI 中日互译：
...
Bot 根据 Bestdori 的 T10 逐分钟 PT 采样估算火量。普通活动每次 PT 上涨视为打完一把，消耗 3 火；组曲活动每次 PT 上涨代表完成 3 首歌，消耗 9 火。普通活动剩余 3 到 5 火时提醒，组曲剩余 9 到 17 火时提醒。Bot 会在启动命令所在频道 @主跑；如果本服务器已通过 `/lb join` 连接语音且朗读开启，还会同时语音提醒。

| 命令 | 作用 |
| --- | --- |
| `/fire start rank:<1-10> [fire] [runner]` | 按当前 T10 名次开始监控；火量默认 99，也可填 0 到 99 的任意整数 |
| `/fire refill method:<补火方式> [amount] [runner]` | 火罐直接补到 99；星石增加 10 的倍数，默认增加 90 |
| `/fire set amount:<火量> [runner]` | 手动校正火量，并清除待补算局数 |
| `/fire status [runner]` | 查看当前火量、游戏 UID 和会话状态 |
| `/fire stop [runner]` | 提前停止补火监控 |

普通活动剩余火量低于 3、组曲活动低于 9 时，Bot 会提供“火罐补到 99”“星石增加 90”和“手动填写当前火量”三个同色按钮。手动填写用于录入现在实际剩余的 0 到 99 火。星石从当前剩余火量上增加，因此剩余 2 火时会补到 92。等待确认期间的新 PT 上涨会在后台记录，不再额外发送公开消息；选择火罐或星石后会按当前活动的消耗量自动补扣，手动填写则以录入的实际火量为准并清除待补算次数。按钮操作结果仅向操作者显示，公开提示只会移除按钮。主跑本人可以操作自己的会话，服务器管理员可以代为操作；活动结束 5 分钟后仍未手动停止的会话会自动关闭。

Bot 会同时按游戏 UID 检查玩家等级。每提升一级增加 10 火，最多 99，并重新计算是否还需要补火。

> 该功能只能追踪当前 T10 玩家，无法读取游戏客户端的真实火量。掉出 T10 时会暂停并提示，回榜后自动恢复。自然回火、1/2 火打法，以及单个 Bestdori 采样实际包含多局的情况不会自动推算，请用 `/fire set` 校正。
