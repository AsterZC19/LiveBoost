...
普通活动剩余火量低于 3、组曲活动低于 9 时，Bot 会发出“火罐补到 99”和“星石增加 90”两个确认按钮。星石从当前剩余火量上增加，因此剩余 2 火时会补到 92。等待确认期间的新 PT 上涨仍会记录，确认后按当前活动的消耗量自动补扣。主跑本人可以操作自己的会话，服务器管理员可以代为操作；活动结束 5 分钟后仍未手动停止的会话会自动关闭。

Bot 同时按游戏 UID 检查玩家等级。每提升一级增加 10 火，最多 99，并重新计算是否还需要补火。
