...
- 双推送：
  - 分速推送：按活动类型间隔推送
  - 时速推送：每个整点推送一次
- 补火提醒：根据 T10 PT 变化自动扣除 3 火，并在最后一把前进行文字与语音提示
- 语音 TTS + AI 中日互译：
...
Bot 根据 Bestdori 的 T10 逐分钟 PT 采样估算火量：**每出现一次新的 PT 上涨，视为打完一把并消耗 3 火**。剩余 3 到 5 火时只够最后一把，Bot 会在启动命令所在频道 @主跑；如果本服务器已通过 `/lb join` 连接语音且朗读开启，还会同时语音提醒。
...
剩余火量低于 3 时，Bot 会发出“火罐补到 99”和“星石增加 90”两个确认按钮。星石从当前剩余火量上增加，因此剩余 2 火时会补到 92。等待确认期间的新 PT 上涨仍会记录，确认后自动补扣。主跑本人可以操作自己的会话，服务器管理员可以代为操作；活动结束 5 分钟后仍未手动停止的会话会自动关闭。
