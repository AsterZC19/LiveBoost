...
朗读细节：
- **用户名**按自身语种读（含假名的名字用日文音色、纯汉字的用中文音色），不被后面的消息语种影响。
- **纯 emoji / 颜文字 / 符号消息**不朗读也不翻译，直接跳过；夹杂在文本里的 emoji 和颜文字，朗读时会剥掉，只读文字和标点（翻译回复里仍保留原样）。
- 一个已知限制：纯汉字的名字（如日文名「山田」）无法从字符判断中日，会按中文读；名字带假名则能正确识别。
