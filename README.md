...
普通活动剩余火量低于 3、组曲活动低于 9 时，Bot 会提供“火罐补到 99”“星石增加 90”和“手动填写当前火量”三个同色按钮。手动填写用于录入现在实际剩余的 0 到 99 火。星石从当前剩余火量上增加，因此剩余 2 火时会补到 92。等待确认期间的新 PT 上涨仍会记录；选择火罐或星石后会按当前活动的消耗量自动补扣，手动填写则以录入的实际火量为准并清除待补算次数。主跑本人可以操作自己的会话，服务器管理员可以代为操作；活动结束 5 分钟后仍未手动停止的会话会自动关闭。
