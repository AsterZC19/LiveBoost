...
- **发图片/视频/语音/文件/贴纸会播报**"谁发送了什么"（如"钴蓝音 发送了图片"，日文名用日文播报）；带文字的消息读完文字会补一句"还发送了图片"。
- 已知限制：AI 不可用（未配置 key / 调用失败）时，纯汉字名退化为字符判定（按中文读），混排消息的 emoji 会跳过。

工作方式：绑定后，机器人在**指定文本频道**里收到消息（本机器人自己发的除外，避免循环；**其他 bot 发的消息也会朗读和翻译**），会先调用 AI
识别语种、是否中日混杂，并生成中/日两个完整版本；随后按开关决定是否**朗读原文**（Edge TTS，免费，
混排消息按语言分段用对应音色读）以及**回复翻译**（单语只给另一种语言，混排给中文版+日文版两条在同一回复里）。
朗读按顺序排队：播放时会**预合成下一条**、长文本按句切块并行合成，下一条几乎无等待就接着读。
重启后会自动恢复上次的会话与开关状态。
